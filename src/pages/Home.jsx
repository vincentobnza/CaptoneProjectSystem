    ...
    <section className="w-full space-y-6 max-w-screen-lg mx-auto">
      <div className="max-w-2xl w-full flex flex-col gap-2">
        ...
        <motion.h1
          ...
          whileInView={{ opacity: 1 }}
          transition={{ duration: 0.5, delay: 0.5 }}
          className="text-5xl text-white font-black leading-snug"
        >
...
const Box = () => {
  const List = [
    ...
    },
    {
      ...
      description: "Display random quotes with JavaScript.",
      icon: "https://cdn-icons-png.flaticon.com/128/14753/14753811.png",
    },
    {
      id: 8,
      title: "Countdown Timer",
      description: "JavaScript countdown timer for events.",
  ...
  return (
    <div className="w-full p-5 max-w-screen-2xl mx-auto rounded-2xl bg-gradient-to-br from-purple-800 via-pink-800 to-indigo-700 flex flex-col gap-2">
      <div className="w-full grid md:grid-cols-3 grid-cols-2 gap-2 md:gap-4">
        {List.map((item) => (
          <div
            key={item.id}
          ...
          >
            <h1 className="text-sm md:text-xl font-black text-white">
              {item.title}
            </h1>
            <p className="text-xs font-medium z-10">{item.description}</p>

            <img
              src={item.icon}
              className="w-30 absolute -right-10 -bottom-10 opacity-40 group-hover:scale-150 duration-500 "
            />
          </div>
        ))}
      </div>
...
const Percent = () => {
  return (
    <div className="w-full max-w-screen-lg grid place-items-center p-5">
